<style>
    ...
    /* 选中论文的标识 */
    .conference-item.selected {
        position: relative;
    }
    
    .conference-item.selected::before {
        content: '⭐';
        position: absolute;
        top: 10px;
        right: 10px;
...
<h2 style="margin: 60px 0px -15px;">Conference</h2>

<div class="conference-controls">
    <button class="control-btn active" id="showAllBtn" onclick="showAllConference()">Show All</button>
    <button class="control-btn" id="showSelectedBtn" onclick="showSelectedConference()">Show Selected</button>
</div>

<script>
function showAllConference() {
    // 显示所有会议论文
    ...
    document.getElementById('showSelectedBtn').classList.remove('active');
}

function showSelectedConference() {
    // 只显示选中的会议论文
    const items = document.querySelectorAll('.conference-item');
    items.forEach(item => {
    ...
    document.getElementById('showSelectedBtn').classList.add('active');
}
</script>
